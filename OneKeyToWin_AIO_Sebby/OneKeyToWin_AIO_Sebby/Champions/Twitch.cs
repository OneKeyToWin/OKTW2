...
namespace OneKeyToWin_AIO_Sebby.Champions
{
    class Twitch : Base
    {
        ...
        public Twitch()
        {
            ...
            HeroMenu.SubMenu("Q Config").AddItem(new MenuItem("countQ", "Auto Q if x enemies are going in your direction 0-disable", true).SetValue(new Slider(3, 5, 0)));
            HeroMenu.SubMenu("Q Config").AddItem(new MenuItem("autoQ", "Auto Q in combo", true).SetValue(true));
            HeroMenu.SubMenu("Q Config").AddItem(new MenuItem("recallSafe", "Safe Q recall", true).SetValue(true));

            HeroMenu.SubMenu("W Config").AddItem(new MenuItem("autoW", "AutoW", true).SetValue(true));

            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("Eks", "E ks", true).SetValue(true));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("countE", "Auto E if x stacks & out range AA", true).SetValue(new Slider(6, 6, 0)));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("5e", "Always E if 6 stacks", true).SetValue(true));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("jungleE", "Jungle ks E", true).SetValue(true));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("Edead", "Cast E before Twitch die", true).SetValue(true));

            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("Rks", "R KS out range AA", true).SetValue(true));
            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("countR", "Auto R if x enemies (combo)", true).SetValue(new Slider(3, 5, 0)));
        ...
        {
            if (args.Slot == SpellSlot.Recall && Q.IsReady() && MainMenu.Item("recallSafe", true).GetValue<bool>())
            {
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Program.LagFree(2) && Q.IsReady() && !Player.IsWindingUp)
                LogicQ();
            if (Program.LagFree(3) && MainMenu.Item("autoW", true).GetValue<bool>() && W.IsReady() && !Player.IsWindingUp)
                LogicW();
        ...
        {
            if (MainMenu.Item("Edead", true).GetValue<bool>() && E.IsReady() && sender.IsEnemy && sender.IsValidTarget(1500))
            {
                double dmg = 0;
        ...
        private void LogicR()
        {
            ...
            if (t.IsValidTarget())
            {
                if (!Orbwalking.InAutoAttackRange(t) && MainMenu.Item("Rks", true).GetValue<bool>() && Player.GetAutoAttackDamage(t) * 4 > t.Health)
                    R.Cast();

                if (t.CountEnemiesInRange(450) >= MainMenu.Item("countR", true).GetValue<Slider>().Value && 0 != MainMenu.Item("countR", true).GetValue<Slider>().Value)
                    R.Cast();
            }
        }
        ...
        private void LogicQ()
        {
            ...
            if (MainMenu.Item("autoQ", true).GetValue<bool>() && Program.Combo && Orbwalker.GetTarget().IsValid<Obj_AI_Hero>() && Player.Mana > RMANA + QMANA)
                Q.Cast();

            if (MainMenu.Item("countQ", true).GetValue<Slider>().Value == 0 || Player.Mana < RMANA + QMANA)
                return;
            ...
            if (count >= MainMenu.Item("countQ", true).GetValue<Slider>().Value)
                Q.Cast();
        }
        ...
        {
            if (!MainMenu.Item("jungleE", true).GetValue<bool>() || Player.Mana < RMANA + EMANA || Player.Level == 1)
                return;
