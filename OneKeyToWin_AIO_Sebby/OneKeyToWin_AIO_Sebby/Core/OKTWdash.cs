...
namespace OneKeyToWin_AIO_Sebby.Core
{
    class OKTWdash : Program
    {
        ...
        public OKTWdash(Spell qwer)
        {
            ...
            HeroMenu.SubMenu(qwer.Slot + " Config").AddItem(new MenuItem("DashMode", "Dash MODE", true).SetValue(new StringList(new[] { "Game Cursor", "Side", "Safe position" }, 2)));
            HeroMenu.SubMenu(qwer.Slot + " Config").AddItem(new MenuItem("EnemyCheck", "Block dash in x enemies ", true).SetValue(new Slider(3, 5, 0)));
            HeroMenu.SubMenu(qwer.Slot + " Config").AddItem(new MenuItem("WallCheck", "Block dash in wall", true).SetValue(true));
            HeroMenu.SubMenu(qwer.Slot + " Config").AddItem(new MenuItem("TurretCheck", "Block dash under turret", true).SetValue(true));
            HeroMenu.SubMenu(qwer.Slot + " Config").AddItem(new MenuItem("AAcheck", "Dash only in AA range", true).SetValue(true));

            HeroMenu.SubMenu(qwer.Slot + " Config").SubMenu("Gapcloser").AddItem(new MenuItem("GapcloserMode", "Gapcloser MODE", true).SetValue(new StringList(new[] { "Game Cursor", "Away - safe position", "Disable" }, 1)));
            foreach (var enemy in HeroManager.Enemies)
                HeroMenu.SubMenu(qwer.Slot + " Config").SubMenu("Gapcloser").AddItem(new MenuItem("EGCchampion" + enemy.ChampionName, enemy.ChampionName, true).SetValue(true));
        ...
        {
            if (DashSpell.IsReady() && MainMenu.Item("EGCchampion" + gapcloser.Sender.ChampionName, true).GetValue<bool>())
            {
                int GapcloserMode = MainMenu.Item("GapcloserMode", true).GetValue<StringList>().SelectedIndex;
                if (GapcloserMode == 0)
                {
        ...
        {
            int DashMode = MainMenu.Item("DashMode", true).GetValue<StringList>().SelectedIndex;
        ...
        {
            if (!MainMenu.Item("AAcheck", true).GetValue<bool>())
                return true;
        ...
        {
            if (MainMenu.Item("WallCheck", true).GetValue<bool>())
            {
            ...
            if (MainMenu.Item("TurretCheck", true).GetValue<bool>())
            {
                if(dashPos.UnderTurret(true))
                    return false;
            }

            var enemyCheck = MainMenu.Item("EnemyCheck", true).GetValue<Slider>().Value;
