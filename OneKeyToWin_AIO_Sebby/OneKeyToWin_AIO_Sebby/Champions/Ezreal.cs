...
namespace OneKeyToWin_AIO_Sebby.Champions
{
    class Ezreal : Base
    {
        ...
        public Ezreal()
        {
            ...
            HeroMenu.SubMenu("Draw").AddItem(new MenuItem("noti", "Show notification", true).SetValue(false));
            ...
            HeroMenu.SubMenu("W Config").AddItem(new MenuItem("wPush", "W on towers", true).SetValue(true));
            HeroMenu.SubMenu("W Config").AddItem(new MenuItem("harassW", "Harass W", true).SetValue(true));

            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("smartE", "SmartCast E key", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press))); //32 == space
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("smartEW", "SmartCast E + W key", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press))); //32 == space
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("EKsCombo", "E ks combo", true).SetValue(true));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("EAntiMelee", "E anti-melee", true).SetValue(true));
            HeroMenu.SubMenu("E Config").AddItem(new MenuItem("autoEgrab", "Auto E anti grab", true).SetValue(true));
            ...
            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("Raoe", "R AOE", true).SetValue(new Slider(3, 5, 0)));
            ...
            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("MaxRangeR", "Max R range", true).SetValue(new Slider(3000, 5000, 0)));
            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("MinRangeR", "Min R range", true).SetValue(new Slider(900, 5000, 0)));
            ...
            HeroMenu.SubMenu("Farm").AddItem(new MenuItem("FQ", "Farm Q out range", true).SetValue(true));
            HeroMenu.SubMenu("Farm").AddItem(new MenuItem("LCP", "FAST LaneClear", true).SetValue(true));

            HeroMenu.AddItem(new MenuItem("debug", "Debug", true).SetValue(false));
        ...
        {
            if (MainMenu.Item("wPush", true).IsActive() && args.Target.Type == GameObjectType.obj_AI_Turret)
            {
            ...
            if (W.IsReady() && Player.Mana > RMANA + WMANA + EMANA)
            {
                ...
                if (target != null)
                {
                    ...
                    else if (Program.Harass && MainMenu.Item("harassW", true).GetValue<bool>() && MainMenu.Item("Harass" + target.ChampionName).GetValue<bool>() && Player.Mana > Player.MaxMana * 0.8 && Player.ManaPercent > MainMenu.Item("HarassMana", true).GetValue<Slider>().Value && OktwCommon.CanHarras())
                    {
                        if (W.Cast(prediction.CastPosition))
                        {
                            args.Process = false;
        ...
        {
            if(sender.IsMe && MainMenu.Item("autoEgrab", true).GetValue<bool>() && E.IsReady())
            {
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            if (R.IsReady() && MainMenu.Item("Rjungle", true).GetValue<bool>())
            {
                KsJungle();
            }
            else
            ...
            if (E.IsReady())
            {
                ...
                if (MainMenu.Item("smartE", true).GetValue<KeyBind>().Active)
                    Esmart = true;
                if (MainMenu.Item("smartEW", true).GetValue<KeyBind>().Active && W.IsReady())
                {
        ...
        private void LogicQ()
        {
            ...
            if (Program.LagFree(1))
            {
                ...
                if (Program.Combo && Player.Mana > RMANA + QMANA)
                {
                    ...
                    if (t.IsValidTarget() && (!W.IsReady() || !MainMenu.Item("autoW", true).GetValue<bool>() || !CanCastSpellPred(W, t)))
                    {
                ...
                foreach (var t in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(Q.Range)).OrderBy(t => t.Health))
                {
                    ...
                    if (cc && (!W.IsReady() || !MainMenu.Item("autoW", true).GetValue<bool>() || !CanCastSpellPred(W, t)) && !OktwCommon.CanMove(t))
                        Q.Cast(t);

                    if (harass && (!W.IsReady() || !MainMenu.Item("autoW", true).GetValue<bool>() || !CanCastSpellPred(W, t)) && MainMenu.Item("Harass" + t.ChampionName).GetValue<bool>())
                        Program.CastSpell(Q, t);
                }
            }
        ...
        private void LogicW()
        {
            ...
            if (t.IsValidTarget())
            {
                ...
                else if (Program.Harass && MainMenu.Item("harassW", true).GetValue<bool>() && MainMenu.Item("Harass" + t.ChampionName).GetValue<bool>() && Player.Mana > Player.MaxMana * 0.8 && Player.ManaPercent > MainMenu.Item("HarassMana", true).GetValue<Slider>().Value && OktwCommon.CanHarras())
                    Program.CastSpell(W, t);
                else
                {
        ...
        private void LogicE()
        {
            ...
            if (MainMenu.Item("EAntiMelee", true).GetValue<bool>())
            { 
            ...
            if (t.IsValidTarget() && Program.Combo && MainMenu.Item("EKsCombo", true).GetValue<bool>() && Player.HealthPercent > 40 && t.Distance(Game.CursorPos) + 300 < t.Position.Distance(Player.Position) && !Orbwalking.InAutoAttackRange(t) && !Player.UnderTurret(true) && (Game.Time - OverKill > 0.3) )
            {
        ...
        private void LogicR()
        {
            ...
            if (MainMenu.Item("autoR", true).GetValue<bool>() && Player.CountEnemiesInRange(800) == 0 && Game.Time - OverKill > 0.6)
            {
                ...
                foreach (var target in HeroManager.Enemies.Where(target => target.IsValidTarget(R.Range) && OktwCommon.ValidUlt(target)))
                {
                    ...
                    if (MainMenu.Item("Rcc", true).GetValue<bool>() && target.IsValidTarget(Q.Range + E.Range) && target.Health < Player.MaxHealth && !OktwCommon.CanMove(target))
                    {
                    ...
                    if (Rdmg > predictedHealth && target.CountAlliesInRange(500) == 0 && Player.Distance(target) > MainMenu.Item("MinRangeR", true).GetValue<Slider>().Value)
                    {
                    ...
                    {
                        R.CastIfWillHit(target, MainMenu.Item("Raoe", true).GetValue<Slider>().Value, true);
                    }
                }
            }
        }
        ...
        private void KsJungle()
        {
            ...
            foreach (var mob in mobs)
            {
                if (mob.Health == mob.MaxHealth)
                    continue;
                if (((mob.SkinName.ToLower().Contains("dragon") && MainMenu.Item("Rdragon", true).GetValue<bool>())
