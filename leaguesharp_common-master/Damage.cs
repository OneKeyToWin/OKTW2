...
*/
        ...
        */
            ...
            double amount)
        {
            ...
            switch (damageType)
            {
                case DamageType.Magical:
                    damage = (double)DamageLibrary.CalculateDamageOnUnit(source, target, DmgLibDamageType.Magical, (float)amount);//CalcMagicDamage(source, target, amount);
                    break;
                case DamageType.Physical:
                    damage = (double)DamageLibrary.CalculateDamageOnUnit(source, target, DmgLibDamageType.Physical, (float)amount);//CalcPhysicalDamage(source, target, amount);
                    break;
        ...
        {
            return DamageLibrary.GetAutoAttackDamage(source, target, includePassive);
            ...
            var sourceMinion = source as Obj_AI_Minion;

            if (sourceMinion != null)
            {
                k += sourceMinion.PercentDamageToBarracksMinionMod;
            }

            var targetMinion = source as Obj_AI_Minion;

            if (targetMinion != null)
            {
                result -= targetMinion.FlatDamageReductionFromBarracksMinionMod;
            }
            ...
            int stage = 0)
        {
            if (Spells.ContainsKey(source.ChampionName))
            {
                ...
                spell.CalculatedDamage = (double)DamageLibrary.GetSpellDamage(source, target, slot, false); // CalcDamage(source, target, spell.DamageType, rawDamage);
                return spell;
            }
            ...
            return new DamageSpell
            {
                CalculatedDamage = (double)DamageLibrary.GetSpellDamage(source, target, slot, false),
                Slot = slot,
                Stage = stage
            };
        }
