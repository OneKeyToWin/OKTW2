...
namespace OneKeyToWin_AIO_Sebby.Champions
{
    class Varus : Base
    {
        ...
        public Varus()
        {
            ...
            HeroMenu.SubMenu("Q Config").AddItem(new MenuItem("maxQ", "Cast Q only max range", true).SetValue(true));
            HeroMenu.SubMenu("Q Config").AddItem(new MenuItem("fastQ", "Fast cast Q", true).SetValue(false));
            ...
            HeroMenu.SubMenu("R Config").AddItem(new MenuItem("rCount", "Auto R if enemies in range (combo mode)", true).SetValue(new Slider(3, 0, 5)));
        ...
        {
            if (R.IsReady() && MainMenu.Item("GapCloser" + gapcloser.Sender.ChampionName).GetValue<bool>())
            {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Program.LagFree(1) && E.IsReady() && MainMenu.Item("autoQ", true).GetValue<bool>() && !Player.IsWindingUp)
                LogicE();
            if (Program.LagFree(2) && Q.IsReady() && MainMenu.Item("autoE", true).GetValue<bool>() && !Player.IsWindingUp)
                LogicQ();
            if (Program.LagFree(3) && R.IsReady() && MainMenu.Item("autoR", true).GetValue<bool>())
                LogicR();
        ...
        private void Farm()
        {
            if (Program.LaneClear && E.IsReady() && MainMenu.Item("farmE", true).GetValue<bool>())
            {
        ...
        private void LogicR()
        {
            foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(R.Range)))
            {
                ...
                if (enemy.CountEnemiesInRange(400) >= MainMenu.Item("rCount", true).GetValue<Slider>().Value && MainMenu.Item("rCount", true).GetValue<Slider>().Value > 0)
                {
            ...
            {
                foreach (var target in HeroManager.Enemies.Where(target => target.IsValidTarget(270) && target.IsMelee && MainMenu.Item("GapCloser" + target.ChampionName).GetValue<bool>()))
                {
